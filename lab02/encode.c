...
void *encode(request_t *request, void *buff)//src,dest
{
    ...
    ((request_t *)buff)->service_name[MAX_SERVICE_NAME_LEN + 1] = '\0'; //make last item in string NULL
    int length = strlen(request->service_name);                    //find the length up to the first NULL
    memset((((request_t *)buff)->service_name + length), '\0', MAX_SERVICE_NAME_LEN - length);       //NULL fill from first NULL to end
